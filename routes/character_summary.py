...
def api_spell_details(spell_name):
    ...
    filename = spell_name.lower().replace(" ", "_").replace("'", "").replace("-", "_") + ".json"
...
def api_spell_management_data():
    ...
    try:
        ...
        always_prepared = []
        if 'spells' in builder.character_data and 'always_prepared' in builder.character_data['spells']:
            for spell_name, spell_data in builder.character_data['spells']['always_prepared'].items():
        ...
        if class_name:
            spell_list_file = Path(__file__).parent.parent / "data" / "spells" / "class_lists" / f"{class_name.lower()}.json"
        ...
        if class_name:
            ...
            if spell_list_file.exists():
                with open(spell_list_file, 'r') as f:
                    spell_list_data = json.load(f)
                    for level_str, spell_names in spell_list_data.get('spells_by_level', {}).items():
                        level = int(level_str)
        ...
        current_selections = {
            'cantrips': list(prepared.get('cantrips', {}).keys()) if isinstance(prepared.get('cantrips', {}), dict) else [],
            'spells': list(prepared.get('spells', {}).keys()) if isinstance(prepared.get('spells', {}), dict) else [],
            'background_cantrips': [],
            'background_spells': []
        }
        ...
        if stats.get('background_spell_requirements'):
            ...
            if bg_req.get('cantrips_needed', 0) > 0:
                ...
                if bg_class:
                    bg_spell_list_file = Path(__file__).parent.parent / "data" / "spells" / "class_lists" / f"{bg_class.lower()}.json"
                ...
                if isinstance(background_spells, dict):
                    current_selections['background_cantrips'] = [
                        name for name, data in background_spells.items()
                ...
                if bg_class:
                    ...
                    if bg_spell_list_file.exists():
                        with open(bg_spell_list_file, 'r') as f:
                            ...
                            spell_names = bg_spell_list_data.get('spells_by_level', {}).get('1', [])
                            for spell_name in spell_names:
                                bg_spells.append({
                                    'name': spell_name,
                                    'school': 'Unknown'
...
def api_mastery_management_data():
    ...
    try:
        ...
        weapons_file = Path(__file__).parent.parent / "data" / "equipment" / "weapons.json"
        weapon_masteries = {}
            ...
            with open(weapons_file, 'r') as f:
                ...
                for weapon_name in stats.get('available_weapons', []):
                    if weapon_name in weapons_data:
                        weapon_masteries[weapon_name] = weapons_data[weapon_name].get('mastery', 'Unknown')
...
def api_save_mastery_selections():
    ...
    try:
        ...
        if len(masteries) > max_masteries:
            return jsonify({"error": f"Too many masteries selected. Maximum is {max_masteries}"}), 400
