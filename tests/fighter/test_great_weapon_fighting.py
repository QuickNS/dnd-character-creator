...
class TestGreatWeaponFighting:
    ...
    def test_gwf_increases_greatsword_average_damage(
        ...
        greatsword_gwf = next((a for a in attacks_gwf if a["name"] == "Greatsword"), None)
        ...
        greatsword_no_gwf = next((a for a in attacks_no_gwf if a["name"] == "Greatsword"), None)
    ...
    def test_gwf_only_applies_to_two_handed_weapons(self, fighter_with_gwf):
        ...
        for attack in attacks:
            ...
            has_gwf_note = any("GWF" in note or "reroll" in note.lower() for note in damage_notes)
            ...
            if weapon_name == "Greatsword":
                assert has_gwf_note, f"{weapon_name} (Two-Handed melee) should have GWF note"
            elif weapon_name == "Spear":
                assert has_gwf_note, f"{weapon_name} (Versatile melee) should have GWF note"
            elif weapon_name == "Shortbow":
                assert not has_gwf_note, f"{weapon_name} (ranged) should NOT have GWF note"
    ...
    def test_gwf_in_character_export(self, fighter_with_gwf):
        ...
        assert avg_damage == 11.3, f"Expected exported avg damage 11.3, got {avg_damage}"
