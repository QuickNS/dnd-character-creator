...
class TestEquipmentLoading:
    ...
    def test_fighter_option_b_equipment(self, data_loader):
        ...
        assert 'Studded Leather Armor' in items, "Fighter option B should include Studded Leather"
...
class TestWeaponMastery:
    ...
    def test_simple_weapons_have_mastery(self, weapons):
        ...
        simple_weapons = ['Club', 'Dagger', 'Mace', 'Quarterstaff', 'Spear', 
                         'Light Crossbow', 'Shortbow']
        
        for weapon_name in simple_weapons:
            assert weapon_name in weapons, f"{weapon_name} should be in weapons database"
            weapon = weapons[weapon_name]
            assert 'mastery' in weapon, f"{weapon_name} should have mastery property"
            assert weapon['mastery'] is not None, f"{weapon_name} mastery should not be None"
            assert len(weapon['mastery']) > 0, f"{weapon_name} mastery should not be empty"
    
    def test_martial_weapons_have_mastery(self, weapons):
        """Test that martial weapons have mastery properties."""
        martial_weapons = ['Longsword', 'Greatsword', 'Battleaxe', 'Rapier', 
                          'Scimitar', 'Shortsword', 'Longbow', 'Heavy Crossbow']
    ...
    def test_mastery_values_are_valid(self, weapons):
        ...
        valid_masteries = ['Nick', 'Vex', 'Slow', 'Push', 'Topple', 'Sap', 
                          'Cleave', 'Graze']
