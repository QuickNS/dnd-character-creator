...
- **Per-row pending indicator.** Each class row in the list shows an amber dot when that row has unresolved choices. A row is "pending" when:
  ...
  - `is_primary == false` and the filtered secondary-row `nested_choices` (skill / tool picks narrowed by multiclass rules, plus preserved level-1 feature choices) are unsatisfied.
