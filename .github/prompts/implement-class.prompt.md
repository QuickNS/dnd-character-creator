...
This details the execution plan for implementing the class features for class {{ class_name }} in accordance with:
...
1. Check the data files located at data/classes/{{ class_name }}.yaml and data/subclasses/ for any inconsistencies or missing information related to the wiki page for {{ class_name }}. The wiki information is stored in wiki_data/classes/{{ class_name }}.json. If needed, update the data files to ensure they accurately reflect the wiki content.
