...
class LevelManager:
    ...
    def get_class_features_at_level(self, class_name: str, level: int) -> Dict[str, str]:
    ...
    def get_all_features_up_to_level(self, class_name: str, level: int) -> Dict[int, Dict[str, str]]:
    ...
    def get_subclass_data(self, class_name: str, subclass_name: str) -> Optional[Dict[str, Any]]:
    ...
    def get_subclass_features_at_level(self, class_name: str, subclass_name: str, level: int) -> Dict[str, str]:
    ...
    def get_spell_slots_at_level(self, class_name: str, level: int, subclass_name: str = None) -> Dict[str, int]:
    ...
    def apply_level_progression(self, character, new_level: int) -> None:
        ...
        for level in range(old_level + 1, new_level + 1):
            class_features = self.get_class_features_at_level(character.class_name, level)
        ...
        if hasattr(character, 'spell_slots'):
            if hasattr(character, 'subclass') and self.is_subclass_caster(character.class_name, character.subclass):
    ...
    def display_level_progression(self, class_name: str, start_level: int = 1, end_level: int = 20) -> None:
        ...
        for level in range(start_level, end_level + 1):
            ...
            if level_features:
                ...
                for feature_name, feature_desc in level_features.items():
                    ...
                    short_desc = feature_desc[:60] + "..." if len(feature_desc) > 60 else feature_desc
                    print(f"  • {feature_name}: {short_desc}")
                
                # Show spell slots if available
                spell_slots = self.get_spell_slots_at_level(class_name, level)
                if spell_slots:
                    slots_str = ", ".join([f"{slot_level}: {count}" for slot_level, count in spell_slots.items()])
    ...
    def get_level_summary(self, class_name: str, level: int, subclass_name: str = None) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "spell_slots": {},
            "spells_known": {},
            "ability_score_improvements": self.get_ability_score_improvements(class_name, level)
        }
        ...
        if subclass_name and self.can_choose_subclass(class_name, level):
            ...
            for feat_level in range(self.get_subclass_level(class_name), level + 1):
                level_features = self.get_subclass_features_at_level(class_name, subclass_name, feat_level)
                if level_features:
                    subclass_features[feat_level] = level_features
            summary["subclass_features"] = subclass_features
        
        # Get spell information
        summary["spell_slots"] = self.get_spell_slots_at_level(class_name, level, subclass_name)
