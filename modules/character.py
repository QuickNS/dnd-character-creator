...
class Character:
    ...
    def calculate_hp_bonus(self) -> int:
        ...
        constitution_score = self.ability_scores.final_scores.get("Constitution", 10)
        constitution_bonus = self.hp_calculator.calculate_constitution_bonus(constitution_score, self.level)
        hp_bonuses = self.feature_bonuses.get("hit_points", [])
        feature_bonus = self.hp_calculator.calculate_feature_bonuses(hp_bonuses, self.level)
    ...
    def _to_dict_basic(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'feature_bonuses': {k: [b.copy() for b in v] for k, v in self.feature_bonuses.items()}
        }
