...
// ---------- Page 2 — Spellcasting & Character Details ----------
...
const LEVEL_TO_ORDINAL: Record<number, string> = {
  1: "1st", 2: "2nd", 3: "3rd", 4: "4th", 5: "5th",
  6: "6th", 7: "7th", 8: "8th", 9: "9th",
};

function Page2({ c }: { c: Char }) {
  ...
  // Spells by level: {"0": [...cantrips], "1": [...], ...}
  const byLevel = rec(c.spells_by_level);

  // Flatten spells in level order (0=cantrips first)
  const spellRows: Array<{ level: string; sp: Record<string, unknown> }> = [];
  Object.keys(byLevel)
    .sort((a, b) => Number(a) - Number(b))
    .forEach((lvl) => {
      arr<Record<string, unknown>>(byLevel[lvl]).forEach((sp) => {
        spellRows.push({ level: lvl, sp });
      });
    });

  const languages = arr<string>(c.languages);
  const alignment = str(c.alignment);

  // Editable right-column fields (not generated by the builder)
  ...
  return (
    <div className="sheet-container">
      ...
      <div className="sheet-fields">
        ...
        {/* ── Spellcasting Stats (top-left box) ── */}
        {hasSpellcasting && (
          <>
            {/* Spellcasting Ability — value inside the "SPELLCASTING ABILITY" header row */}
            <BoxField
              style={{ top: 24, left: 65, width: 108, height: 17, fontSize: 8, textAlign: "left" }}
              value={ability}
            />
        ...
        {/* ── Spell Slots — Total values on the underlines (Level 1-9) ── */}
        {Object.entries(slots).map(([key, count]) => {
          const pos = SLOT_POS[key];
          if (!pos) return null;
          const n = num(count);
          if (n === undefined || n === 0) return null;
          const [top, left, width] = pos;
          return (
            <BoxField
              key={`slot-${key}`}
              style={{ top, left, width, height: 16, fontSize: 8 }}
              value={n}
            />
          );
        })}

        {/* Pact Magic slot totals (Warlock) mapped onto the standard level rows */}
        {pactMagicSlots.map((entry, i) => {
          const slotLevel = num(entry.slot_level);
          const slotCount = num(entry.slots);
          if (slotLevel === undefined || slotCount === undefined || slotCount === 0) return null;
          const ordinal = LEVEL_TO_ORDINAL[slotLevel];
          const pos = ordinal ? SLOT_POS[ordinal] : undefined;
          if (!pos) return null;
          const [top, left, width] = pos;
          return (
            <BoxField
              key={`pact-${slotLevel}-${i}`}
              style={{ top, left, width, height: 16, fontSize: 8 }}
              value={slotCount}
            />
          );
        ...
        {spellRows.slice(0, MAX_ROWS).map(({ level, sp }, i) => {
        ...
        {/* Equipment — editable free-text area */}
        <EditableBoxField
          style={{ top: 569, left: 648, width: 132, height: 265, fontSize: 7, textAlign: "left", whiteSpace: "pre-wrap" }}
          value={equipmentText}
          onCommit={setEquipmentText}
        />
